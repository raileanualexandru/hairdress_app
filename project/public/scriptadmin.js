...
const renderCalendar = () => {
    ...
    for (let i = 1; i <= lastDay; i++) {
        ...
        if (i === new Date().getDate() && date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
            days += `<a href="/adminarea?date=${i}/${date.getMonth() + 1}/${date.getFullYear()}"><div class="today">${i}</div></a>`;
        } else {
            //check if is weekend
            if (is_weekend(t) === true) {
               
                    days += `<a href="/adminarea?date=${i}/${date.getMonth() + 1}/${date.getFullYear()}"><div class="weekend">${i}</div></a>`;
                
            } else {
                
                    days += `<a href="/adminarea?date=${i}/${date.getMonth() + 1}/${date.getFullYear()}"><div class="current">${i}</div></a>`;
